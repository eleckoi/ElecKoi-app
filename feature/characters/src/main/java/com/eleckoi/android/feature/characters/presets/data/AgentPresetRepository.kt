...
import com.eleckoi.android.feature.characters.presets.model.defaultAgentPreset
...
import com.eleckoi.android.feature.characters.presets.data.importing.AgentPresetImportCoordinator
import com.eleckoi.android.feature.characters.presets.data.library.AgentPresetLibraryGroupCoordinator
import com.eleckoi.android.feature.characters.presets.data.media.AgentPresetAuthorAvatarStore
import com.eleckoi.android.feature.characters.presets.data.media.detectAgentPresetAvatarMediaType
import com.eleckoi.android.feature.characters.presets.data.policy.uniqueAgentPresetName
import com.eleckoi.android.feature.characters.presets.data.storage.AgentPresetMetadataCodec
import com.eleckoi.android.feature.characters.presets.data.storage.RegexRulesContentKind
import com.eleckoi.android.feature.characters.presets.data.storage.toStorageRecord
import com.eleckoi.android.feature.characters.presets.data.storage.toAgentPreset
import com.eleckoi.android.feature.characters.presets.data.storage.toVersionRecord
import com.eleckoi.android.feature.characters.presets.data.storage.toStandaloneRecord
...
import com.eleckoi.android.foundation.storage.room.AgentPresetStateEntity
...
class AgentPresetRepository(
    ...
    private val authorAvatars = AgentPresetAuthorAvatarStore(store)
    private val importer = AgentPresetImportCoordinator(dao, authorAvatars)
    private val libraryGroups = AgentPresetLibraryGroupCoordinator(dao)
    @Volatile
    private var initialized = false

    val catalog: Flow<AgentPresetCatalog> = combine(
        dao.stateFlow(),
        dao.libraryGroupsFlow(),
        dao.summariesFlow(),
    ) { state, groupRows, rows ->
        val groups = groupRows.map { AgentPresetLibraryGroup(it.id, it.name, it.sortIndex) }
        val knownGroupIds = groups.mapTo(mutableSetOf()) { it.id }
        val summaries = rows.map { row ->
            AgentPresetSummary(
                id = row.id,
                name = row.name,
                modelFamily = AgentPresetModelFamily.fromStorage(row.modelFamily),
                modelTags = AgentPresetMetadataCodec.decodeModelTags(row.modelTagsJson, row.modelFamily),
    ...
    suspend fun ensureInitialized() {
        ...
        initializationMutex.withLock {
            ...
            if (!dao.presetExists(DefaultAgentPresetId)) {
                val preset = defaultAgentPreset().copy(
            ...
            if (currentState?.activePresetId != validActiveId) {
                dao.upsertState(AgentPresetStateEntity(activePresetId = validActiveId))
            }
            initialized = true
        }
    }

    suspend fun preset(presetId: String): AgentPreset? {
        ensureInitialized()
        return dao.preset(presetId)?.let { record ->
            record.toAgentPreset().copy(
                activeVersionNumber = dao.versionNumber(presetId, record.preset.activeVersionId) ?: 1,
            )
        }
    }

    suspend fun activePreset(): AgentPreset {
        ensureInitialized()
        val activeId = dao.state()?.activePresetId ?: DefaultAgentPresetId
        return preset(activeId) ?: defaultAgentPreset()
    }

    suspend fun setActive(presetId: String) {
        ensureInitialized()
        if (dao.state()?.activePresetId != presetId && dao.presetExists(presetId)) {
            dao.upsertState(AgentPresetStateEntity(activePresetId = presetId))
    ...
    suspend fun create(
        ...
        val preset = AgentPreset(
            id = id,
            name = uniqueAgentPresetName(name.trim().ifBlank { "新预设" }, dao.presetNames()),
    ...
    private fun AgentPreset.normalizedForStorage(): AgentPreset {
    ...
    suspend fun rename(presetId: String, name: String) {
        ...
        val normalizedName = uniqueAgentPresetName(
    ...
    suspend fun duplicate(presetId: String): AgentPreset? {
        ensureInitialized()
        val source = dao.preset(presetId)?.toAgentPreset() ?: return null
        val copyId = "agent-preset-${System.currentTimeMillis()}"
        ...
        val copy = source.copy(
            id = copyId,
            name = uniqueAgentPresetName("${source.name} 副本", dao.presetNames()),
    ...
    internal suspend fun importPreset(conversion: AgentPresetImportConversion): AgentPreset {
        ensureInitialized()
        return importer.import(conversion)
    }
    ...
    suspend fun exportPreset(
        ...
        ensureInitialized()
        val record = dao.preset(presetId) ?: error("找不到要导出的预设")
        val preset = record.toAgentPreset().copy(
            activeVersionNumber = dao.versionNumber(presetId, record.preset.activeVersionId) ?: 1,
        )
        val avatar = preset.profile.authorAvatarPath
            .takeIf(String::isNotBlank)
            ?.let(::File)
            ?.takeIf { it.isFile && it.length() in 1..MaxAgentPresetAvatarBytes }
            ?.readBytes()
            ?.let { bytes ->
                bytes.detectAgentPresetAvatarMediaType()?.let { mediaType ->
                    AgentPresetTransferAvatar(mediaType, bytes)
                }
            }
        val versions = dao.versions(presetId).map { version ->
            AgentPresetTransferVersion(
                id = version.version.versionId,
                number = version.version.versionNumber,
                name = version.version.name,
                createdAtEpochMs = version.version.createdAtEpochMs,
                preset = version.toStandaloneRecord(record.preset).toAgentPreset(),
            )
        }
        return AgentPresetCardExporter.export(preset, format, avatar, versions)
    }
        ...
        val presets = dao.presetIds().mapNotNull { id ->
            preset(id)?.let { value ->
                ...
                JSONObject()
                    ...
                    .put("payload", JSONObject(AgentPresetImportCodec.encodeElecKoi(value)))
            }
        }
        return JSONObject()
            .put("format", "eleckoi.agent-presets-backup")
            .put("version", 1)
    ...
    suspend fun restoreBackupJson(json: String) {
        ...
        val root = runCatching { JSONObject(json) }
            .getOrElse { throw IllegalArgumentException("Agent 预设备份已损坏", it) }
        require(root.optString("format") == "eleckoi.agent-presets-backup") {
            "Agent 预设备份格式不正确"
        }
        require(root.optInt("version", -1) == 1) { "不支持的 Agent 预设备份版本" }
        ...
        for (index in 0 until presetArray.length()) {
            ...
            val conversionWithAvatar = conversion.copy(
                authorAvatar = authorAvatar?.let { AgentPresetTransferAvatar("image/png", it) }
                    ?: conversion.authorAvatar,
            )
            val imported = if (sourceId == DefaultAgentPresetId) {
                val current = defaultAgentPreset().copy(
                    id = DefaultAgentPresetId,
                    activeVersionId = "$DefaultAgentPresetId:v1",
                )
                update(conversion.preset.copy(id = current.id, activeVersionId = current.activeVersionId))
                preset(DefaultAgentPresetId) ?: current
            } else {
                importPreset(conversionWithAvatar)
            }
    ...
    suspend fun createLibraryGroup(name: String): AgentPresetLibraryGroup {
        ensureInitialized()
    ...
    suspend fun updateProfile(presetId: String, profile: AgentPresetProfile) {
        ensureInitialized()
        if (!dao.presetExists(presetId)) return
        val normalized = profile.copy(
            authorName = profile.authorName.trim().take(40),
            usageInstructions = profile.usageInstructions.trim().take(8_000),
            ...
            }.distinctBy(AgentPresetTimelineItem::id).take(100),
        )
        dao.updatePresetProfile(
            presetId = presetId,
            authorName = normalized.authorName,
            usageInstructions = normalized.usageInstructions,
            timelineJson = AgentPresetMetadataCodec.encodeTimeline(normalized.timeline),
        )
    }

    suspend fun updateModelTags(presetId: String, modelTags: List<AgentPresetModelTag>) {
        ensureInitialized()
        if (!dao.presetExists(presetId)) return
        val normalizedTags = modelTags
            .map { it.copy(id = it.id.trim().lowercase(), label = it.label.trim()) }
        ...
        dao.updatePresetModelTags(
            ...
            modelTagsJson = AgentPresetMetadataCodec.encodeModelTags(normalizedTags),
        )
    }
    ...
    suspend fun delete(presetId: String) {
        ensureInitialized()
        if (presetId == DefaultAgentPresetId || !dao.presetExists(presetId)) return
        val wasActive = dao.state()?.activePresetId == presetId
        dao.deletePreset(presetId)
        if (wasActive) {
            dao.upsertState(
                AgentPresetStateEntity(
                    activePresetId = dao.firstPresetId() ?: DefaultAgentPresetId,
                ),
            )
            onActivePresetChanged()
        }
...
private const val MaxAgentPresetAvatarBytes = 8L * 1024L * 1024L
